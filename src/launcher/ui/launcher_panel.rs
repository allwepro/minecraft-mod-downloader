...
impl LauncherPanel {
    ...
    pub fn show(
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            egui::ScrollArea::vertical().show(ui, |ui| {
                self.render_content(ui, config_manager, rt_handle, current_list_id, download_dir, selected_loader);
            });
        });
    }
    ...
    fn render_content(
        ...
        ui.group(|ui| {
            ...
            if let Some(ref mc) = self.minecraft_installation {
                ...
                if !mc.available_versions.is_empty() {
                    ...
                    ui.colored_label(
                        egui::Color32::YELLOW,
                        "⚠ No Minecraft versions installed",
                    );
                }
            } else {
        ...
        ui.group(|ui| {
            ...
            let mod_lists = rt_handle.block_on(async {
                config_manager
                    .load_all_lists()
                    .await
                    .unwrap_or_default()
    ...
    fn launch_minecraft(
        ...
        match AdvancedLauncher::launch_minecraft(&config) {
            Ok(LaunchResult::Success { pid }) => {
                self.launch_status =
                    Some(format!("✅ Minecraft launched successfully! (PID: {})", pid));
            }
