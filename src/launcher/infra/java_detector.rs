...
impl JavaDetector {
    ...
    fn get_search_paths() -> Vec<PathBuf> {
        ...
        // Add local java_runtimes directory
        if let Ok(cwd) = std::env::current_dir() {
            paths.push(cwd.join("java_runtimes"));
        }
        ...
        for base in base_paths {
            ...
            if let Ok(entries) = std::fs::read_dir(&base) {
                for entry in entries.flatten() {
                    let path = entry.path();
                    if path.is_dir() {
                        // Look for bin/java or bin/java.exe (Standard structure)
                        let java_bin = path.join("bin").join(Self::java_executable());
                        if java_bin.exists() {
                            java_paths.push(java_bin);
                        }

                        // Check for macOS .jdk bundles (e.g. Contents/Home/bin/java)
                        if cfg!(target_os = "macos") {
                            let mac_bin =
                                path.join("Contents/Home/bin").join(Self::java_executable());
                            if mac_bin.exists() {
                                java_paths.push(mac_bin);
                            }
                        }
                    }
                }
